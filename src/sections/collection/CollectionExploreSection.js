...
import { GetAllCollections } from "../../apis";
...
function CollectionExploreSection() {
  ...
  console.log(state.bidsFilter, "bidsFilter");
  ...
  const CollectinApiFunc = async () => {
    ...
    if (state.bidsFilter !== "all") {
      Object.assign(payloadData, {
        time: JSON.stringify(state.bidsFilter),
      });
    }

    const result = await GetAllCollections(queryParamFormatter(payloadData));
  ...
  const handleTrending = async () => {
    ...
    const result = await GetAllCollections(queryParamFormatter(payloadData)); // functin call axios api
  ...
  // const handleTimedCollection = async (time) => {
  // 	const payload = {
  // 		likes: "desc",
  // 		skip: "0",
  // 		limit: "12",
  ...
  // 	const result = await GetAllCollections(queryParamFormatter(payload, time)); // functin call axios api
  ...
  // React.useEffect(() => {
  // 	handleTimedCollection(time);
  // }, [time]);

  // React.useEffect(() => {
  // 	handleTimedCollection(state.bidsFilter);
  ...
  return (
    ...
    >
      ...
      >
        <CardHeadingGradient text="Explore Collections" />
      </Box>

      <Box maxWidth="500px" pt="12px" pb="16px">
        <Typography variant="h3Light">
          Explore all nft Collections. You can view and explore Collections
          related to multiple categories here.
        </Typography>
      </Box>
      ...
      >
        <CardHeading text={"Top Collections in"}>
          <Box
            component="span"
...
export default CollectionExploreSection;
