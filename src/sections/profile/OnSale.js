...
function OnSale() {
  ...
  const [onSaledNftData, setOnSaleNftData] = React.useState([]);
  ...
  const onSaleNftsData = async (tokenId) => {
    const payload = {
      views: "desc",
      likes: "desc",
      skip: 0,
    ...
    const result = await GetOnsaleNft(
      tokenId,
      queryParamFormatter({ ...payloadData, address: id })
    );
  ...
  const handleMore = async () => {
    ...
    const result = await GetOnsaleNft(token, queryParamFormatter(payloadData));
  ...
  React.useEffect(async () => {
    if (token) {
      setSkip(0);
      onSaleNftsData(token);
    }
  }, [id, state.appyFilter, state.saleFilter, state.priceFilter]);
