...
function Owned() {
  ...
  const getOwnedNfts = async (tokenId) => {
    const payload = {
      views: "desc",
      likes: "desc",
      skip: 0,
    ...
    const result = await ownedNfts(tokenId, queryParamFormatter(payloadData));
  ...
  const handleMore = async () => {
    ...
    const result = await ownedNfts(token, queryParamFormatter(payloadData));
